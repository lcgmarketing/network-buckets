 ...
 * see truethemes_framework/global/basic.php
 *
 * @since 2.3.1
 *
...
//crop image function from truethemes_framework/global/basic.php
...
/* --- EXTRA FULL WIDTH -  BANNER --- */
if ($size == 'banner_extra_full'){

//function from image frames processor, see above.
//used to generate the html content of this shortcode.
//applies to all sizes, including links or not.

$output .= truethemes_image_frame_constructor($style,"_preload_full",$image_path,978,225,$framesize,$link_to_page,$target,$description);
}
...
if(!empty($link_to_page)){
$output.='<img src="'.TRUETHEMES_HOME.'/images/_global/img-zoom-link-'.$image_zoom_number.'.png" style="position:absolute; display: none;" alt="'.$description.'" />';
}{
$output.='<img src="'.TRUETHEMES_HOME.'/images/_global/img-zoom-'.$image_zoom_number.'.png" style="position:absolute; display: none;" alt="'.$description.'" />';
}
...
/*
* New Accordion shortcode class (backward compatible for customer)
* @since version 2.6
*
* handles printing of jquery custom ui script to footer 
* file located in truethemes_framework/js/jquery-ui-1.8.15.custom.min.js
* handles creation of all shortcodes needed for accordion
* handles dynamic creation of individual accordion jquery init code.
*
* note this class only hooks javascript when shortcode is used.
* original concept by Scribu http://scribu.net/wordpress/optimal-script-loading.html
*
* How to use?
...
class truethemes_accordion_class{
	static $add_script;
 
	function init() {
	
	    add_shortcode('accordion', array(__CLASS__, 'accordion_wrapper_shortcode'));
	    add_shortcode('slide', array(__CLASS__, 'accordion_slide_shortcode'));		
	...
	function accordion_wrapper_shortcode( $atts, $content = null ){
		...
		extract(shortcode_atts(array(
  		...
  		'active' => '', //active tab, first is 1, 2, etc 
  		), $atts));
		...
		$active = $active-1;	
		$output .= "[raw]{ active: ".$active.", autoHeight: false, header: \".opener\", collapsible: true, event: \"click\"}[/raw]";
		}
	...
	//note, script already registered in global/javascript.php
	//so that it can be used here!
		if ( ! self::$add_script )
			return;
 
		wp_print_scripts('truethemes-custom-jquery-ui');
	}
}
//initialize the class. 
truethemes_accordion_class::init();




/*
* New Tabs shortcode class (backward compatible for customer)
* @since version 2.6
*
* handles printing of jquery custom ui script to footer 
* file located in truethemes_framework/js/jquery-ui-1.8.15.custom.min.js
* handles creation of all shortcodes needed for tabs
* handles dynamic creation of jquery init code.
*
* note this class only hooks javascript when shortcode is used.
* original concept by Scribu http://scribu.net/wordpress/optimal-script-loading.html
*
...
class truethemes_tab_class{
	static $add_script;
 
	function init() {
		add_shortcode('tabset', array(__CLASS__, 'truethemes_tabset'));
		add_shortcode('tab', array(__CLASS__, 'truethemes_tabs'));
...
function truethemes_notify( $atts, $content = null ) {
  ...
  if($list=='true'):
  return '[raw]<div class="message_'.$style.'" style="font-size:'.$font_size.';">' . do_shortcode($content) . '</div>[/raw]';
  else:
...
$embed_video = $wp_embed->run_shortcode("[embed width='65' height='65']".$video_url."[/embed]");
		...
		elseif(!empty($video_url)):
		$output .= '[raw]<div class="blog-posts-shortcode-thumb">[/raw]';
		$post_thumb .= '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-video-post-small.jpg" width="65" height="65" alt="'.get_the_title().'"/></a>';	
		$output .= $post_thumb;
		$output .= '[raw]</div>[/raw]';
		...
		$output .= '[raw]<div class="blog-posts-shortcode-thumb">[/raw]';
		$post_thumb .= '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-blog-post-small.jpg" width="65" height="65" alt="'.get_the_title().'"/></a>';	
		$output .= $post_thumb;
		$output .= '[raw]</div>[/raw]';
		endif;
		...
		//if empty, we just add break tag for spacing.
		$output .= '<p>[raw]<br/>[/raw]</p><br /><a href="'.$truethemeslink.'">'.$link_text.'</a></p>';
				
		else:
		//we show the content string.
		$output .= '<p>[raw]'.$post_content.'...[/raw]</p><br /><a href="'.$truethemeslink.'">'.$link_text.'</a></p>';
...
$embed_video = $wp_embed->run_shortcode("[embed width='437' height='234']".$video_url."[/embed]");
		...
		//video embed
		elseif(!empty($video_url)):	
		$post_thumb .= '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-video-post.jpg" width="437" height="234" alt="'.get_the_title().'"/></a>';
			
			
		//default image	
		else:
			$post_thumb .= '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-blog-post.jpg" width="437" height="234" alt="'.get_the_title().'"/></a>';
...
$output .= '<h4><a href="'.$truethemeslink.'">'.get_the_title().'</a></h4>';
		...
		if(empty($post_content)):
		//if empty content, we replace with br tag to maintain spacing.
$output .= '<p>[raw]<br/>[/raw]</p><br /><a href="'.$truethemeslink.'" class="ka_button small_button"><span>'.$link_text.'</span></a></p>';			
		
		else:
		//There is content string after removing shortcode, we show it.
$output .= '<p>[raw]'.$post_content.'...[/raw]</p><br /><a href="'.$truethemeslink.'" class="ka_button small_button"><span>'.$link_text.'</span></a></p>';		
...
$embed_video = $wp_embed->run_shortcode("[embed width='275' height='145']".$video_url."[/embed]");
		...
		//video embed
		elseif(!empty($video_url)):	
		$post_thumb .= '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-video-post.jpg" width="275" height="145" alt="'.get_the_title().'"/></a>';	
		
		
		
		//default image
		else:
			$post_thumb = '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-blog-post.jpg" width="275" height="145" alt="'.get_the_title().'"/></a>';
...
$output .= '<h4><a href="'.$truethemeslink.'">'.get_the_title().'</a></h4>';
		...
		if(empty($post_content)):
		//emppty post content, we show br tag to maintain spacing
$output .= '<p>[raw]<br/>[/raw]</p><br /><a href="'.$truethemeslink.'" class="ka_button small_button"><span>'.$link_text.'</span></a></p>';
...
$embed_video = $wp_embed->run_shortcode("[embed width='190' height='111']".$video_url."[/embed]");
		...
		//video embed
		elseif(!empty($video_url)):	
		$post_thumb .= '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-video-post.jpg" width="190" height="111" alt="'.get_the_title().'"/></a>';	
		
		
		//default image
		else:
			$post_thumb = '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-blog-post.jpg" width="190" height="111" alt="'.get_the_title().'"/></a>';
...
$embed_video = $wp_embed->run_shortcode("[embed width='324' height='180']".$video_url."[/embed]");
		...
		//video embed
		elseif(!empty($video_url)):	
		$post_thumb .= '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-video-post.jpg" width="324" height="180" alt="'.get_the_title().'"/></a>';
		
		else:
			$post_thumb = '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-blog-post.jpg" width="324" height="180" alt="'.get_the_title().'"/></a>';
...
$embed_video = $wp_embed->run_shortcode("[embed width='202' height='113']".$video_url."[/embed]");
		...
		//video embed
		elseif(!empty($video_url)):	
		$post_thumb .= '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-video-post.jpg" width="202" height="113" alt="'.get_the_title().'"/></a>';
		
				
		else:
			$post_thumb = '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-blog-post.jpg" width="202" height="113" alt="'.get_the_title().'"/></a>';
...
$embed_video = $wp_embed->run_shortcode("[embed width='135' height='76']".$video_url."[/embed]");
		...
		//video embed
		elseif(!empty($video_url)):	
		$post_thumb .= '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-video-post.jpg" width="135" height="76" alt="'.get_the_title().'"/></a>';			
			
		else:
			$post_thumb = '<a href="'.$truethemeslink.'"><img src="'.TRUETHEMES_HOME.'/images/_global/default-blog-post.jpg" width="135" height="76" alt="'.get_the_title().'"/></a>';
